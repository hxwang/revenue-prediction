...
\begin{abstract}
In this project, we worked on predicting the revenues for restaurants which is a competition hosted by Kaggle. The dataset provided by TFI contains a training set of  $137$ restaurant records, and a test set of $100, 000$  restaurant records. We pre-process data to handle with categorical attributes and missing values. We explore the dataset by studying the correlation between features, feature ranking and principal component analysis. We test different models including GradientBoostingRegressor, svm.NuSVR, KNeighborsRegressor, RandomForestRegressor and so on. We find that an ensemble of a subset of these regression models outperforms any of single models. Based on RMSE evaluation metric, our regression model ranked within top $3\%$ of all participated teams in the competition.
\end{abstract}
...
\subsection{Correlation among Features} We further study the correlation among features in the training set and testing set respectively. We plot the results in Figure~\ref{fig_corr_train} and Figure~\ref{fig_corr_test} respectively. Though the correlations among features pairs in test set follow the same pattern of that in training set, features are less correlated which also confirms the PCA result that test set is hard to compress.
...
\begin{figure}[h]
   \centering
   \includegraphics[width=5.6in]{figs/cv_pb_scores.eps} 
...
\begin{figure}[h]
   \centering
   \includegraphics[width=6in]{figs/pb.eps} 
   \caption{RMSE and ranking on the leaderboard.}
   \label{fig:pb}
\end{figure}
